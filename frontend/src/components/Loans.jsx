...
const FormModal = ({ setOpenModal }) => {
  ...
  async function handleSubmit(event) {
    ...
    const response = await fetch(
      `${import.meta.env.VITE_BACKEND_URL}/loan/postloan`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  return (
    <div className="fixed top-0 left-0 w-screen h-[100%] flex items-center justify-center bg-[rgba(0,0,0,0.8)] font-QuickSand font-medium z-50">
      <div className="bg-primaryBlack shadow-boxShadow rounded-lg w-[650px] h-[70%] overflow-y-auto no-scrollbar px-5">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mx-4 my-5">
            <label htmlFor="type" className="block text-gray-400 text-3xl mb-2">
              Type
            </label>
          ...
          <div className="mx-4 my-5">
            <label
              htmlFor="startDate"
              className="block text-gray-400 text-3xl mb-2"
            >
              Start Date
            </label>
            <input
          ...
          <div className="block border border-gray-300 my-8 mx-4"></div>
          <div className="flex items-center justify-end gap-5 px-4 py-2">
            <button
              className="text-2xl border border-gray-300 rounded-md px-4 py-2 text-red-500"
              onClick={() => {
                setOpenModal(false);
              }}
            >
